...
func (c *SatelliteCollector) Collect(ch chan<- prometheus.Metric) {
	for _, client := range c.clients {
		for _, satellite := range client.Satellites {
			...
			ch <- prometheus.MustNewConstMetric(c.storageSummary, prometheus.GaugeValue, float64(satelliteData.StorageSummary), satellite.ID, client.NodeID, satellite.URL)
			ch <- prometheus.MustNewConstMetric(c.averageUsageBytes, prometheus.GaugeValue, float64(satelliteData.AverageUsageBytes), satellite.ID, client.NodeID, satellite.URL)
			ch <- prometheus.MustNewConstMetric(c.bandwidthSummary, prometheus.GaugeValue, float64(satelliteData.BandwidthSummary), satellite.ID, client.NodeID, satellite.URL)
			ch <- prometheus.MustNewConstMetric(c.egressSummary, prometheus.GaugeValue, float64(satelliteData.EgressSummary), satellite.ID, client.NodeID, satellite.URL)
			ch <- prometheus.MustNewConstMetric(c.ingressSummary, prometheus.GaugeValue, float64(satelliteData.IngressSummary), satellite.ID, client.NodeID, satellite.URL)
			ch <- prometheus.MustNewConstMetric(c.currentStorageUsed, prometheus.GaugeValue, float64(satelliteData.CurrentStorageUsed), satellite.ID, client.NodeID, satellite.URL)
			ch <- prometheus.MustNewConstMetric(c.auditScore, prometheus.GaugeValue, float64(satelliteData.Audits.AuditScore), satellite.ID, client.NodeID, satellite.URL)
			ch <- prometheus.MustNewConstMetric(c.suspensionScore, prometheus.GaugeValue, float64(satelliteData.Audits.SuspensionScore), satellite.ID, client.NodeID, satellite.URL)
			ch <- prometheus.MustNewConstMetric(c.onlineScore, prometheus.GaugeValue, float64(satelliteData.Audits.OnlineScore), satellite.ID, client.NodeID, satellite.URL)
			ch <- prometheus.MustNewConstMetric(c.monthlyEgress, prometheus.GaugeValue, float64(totalMonthlyEgress), satellite.ID, client.NodeID, satellite.URL)
			ch <- prometheus.MustNewConstMetric(c.monthlyIngress, prometheus.GaugeValue, float64(totalMonthlyIngress), satellite.ID, client.NodeID, satellite.URL)
			ch <- prometheus.MustNewConstMetric(c.dayStorage, prometheus.GaugeValue, float64(satelliteData.StorageDaily[len(satelliteData.StorageDaily)-1].AtRestTotal), satellite.ID, client.NodeID, satellite.URL)
		}
	}
}
